"""
This module includes CNN models for EEG Depression classification tasks. 
...
class TwoDCNN(nn.Module):
    """
    Ravindran AS, Contreras‐Vidal J H (2023)
    An empirical comparison of deep learning explainability approaches
    for EEG using simulated ground truth
    Nature, Scientifc Reports 13:17709
    doi: https://doi.org/10.1038/s41598-023-43871-8
    """
    def __init__(self, n_class, n_chan, n_samples):
        super(TwoDCNN, self).__init__()

        #Convolutional, pooling and flatten layers
        self.conv1 = nn.Conv2d(in_channels=1, out_channels=32, kernel_size=(5, 1), stride=1, padding='same')
        self.pool1 = nn.MaxPool2d(kernel_size=(2, 1), stride=(2,1))
        self.conv2 = nn.Conv2d(in_channels=32, out_channels=32, kernel_size=(5, 1), stride=1, padding='same')
        self.pool2 = nn.MaxPool2d(kernel_size=(2, 1), stride=(2,1))
        self.conv3 = nn.Conv2d(in_channels=32, out_channels=32, kernel_size=(5, 1), stride=1, padding='same')
        self.pool3 = nn.MaxPool2d(kernel_size=(2, 1), stride=(2,1))
        self.conv4 = nn.Conv2d(in_channels=32, out_channels=32, kernel_size=(5, 1), stride=1, padding='same')
        self.pool4 = nn.MaxPool2d(kernel_size=(2, 1), stride=(2,1))
        self.conv5 = nn.Conv2d(in_channels=32, out_channels=32, kernel_size=(5, 1), stride=1, padding='same')
        self.conv6 = nn.Conv2d(in_channels=32, out_channels=32, kernel_size=(1, n_chan), stride=1, padding='same')
        self.flatten  = nn.Flatten()

        #Linear and dropout layers
        self.fc1  = nn.Linear(in_features= 32*(n_samples//16)*n_chan, out_features=32)
...
class EEGNet(nn.Module):
    """
    Bo Liu1, Hongli Chang, Kang Peng, Xuenan Wang
    An End-to-End Depression Recognition Method Based on EEGNet.
    Front. Psychiatry 13:864393. (2022)
    doi: 10.3389/fpsyt.2022.864393
    """
    def __init__(self, n_class=4, n_chan=62, n_samples=250, dropoutRate=0.5, kernLength=100, F1=8, D=2, F2=16): #Cek norm_rate F2=16, D=2
        super(EEGNet, self).__init__()

        # First convolutional layer (adjusted for 3D input)
        self.conv2d1 = nn.Conv2d(1, F1, (1, kernLength), padding='same', bias=False)
        self.bn1 = nn.BatchNorm2d(F1)
        #self.relu1 = nn.ReLU() #Replacing elu with Relu
        ...
        #Dense layer
        self.flatten = nn.Flatten()
        self.fc = nn.Linear(F2 * ((n_samples // 4) // 8), n_class) #serega
...
class DeprNet(nn.Module):
    def __init__(self,  n_class=4, n_chan=5, n_samples=2000):
        ...
        '''Ayan Seal, Rishabh Bajpai, Jagriti Agnihotri, Anis Yazidi, Enrique Herrera-Viedma and Ondrej Krejcar
        ...
        self.fc1 = nn.Linear(32*n_chan*(n_samples//32), 16) #26624 mumtaz
...
class TempCNN(nn.Module):
    def __init__(self,  n_class=4, n_samples=2000, n_chan=5):
        super(TempCNN, self).__init__()
        '''U. Rajendra Acharyaa, Shu Lih Oha, Yuki Hagiwaraa, Jen Hong Tana, Hojjat Adeli, D. P Subha
        Automated EEG-based screening of depression using deep convolutional neural network
        Computer Methods and Programs in Biomedicine, VOL. 161, 2018
        DOI: 10.1016/j.cmpb.2018.04.012
        '''
        
        self.conv1 = nn.Conv2d(in_channels=1, out_channels=5, kernel_size=(5, 1), stride=1, padding='same')
        self.relu1 = nn.ReLU()
        self.pool1 = nn.MaxPool2d(kernel_size=(2, 1), stride=(2,1))
        self.conv2 = nn.Conv2d(in_channels=5, out_channels=5, kernel_size=(5, 1), stride=1, padding='same')
        self.relu2 = nn.ReLU()
        self.pool2 = nn.MaxPool2d(kernel_size=(2, 1), stride=(2,1))
        self.conv3 = nn.Conv2d(in_channels=5, out_channels=10, kernel_size=(5, 1), stride=1, padding='same')
        self.relu3 = nn.ReLU()
        self.pool3 = nn.MaxPool2d(kernel_size=(2, 1), stride=(2,1))
        self.conv4 = nn.Conv2d(in_channels=10, out_channels=10, kernel_size=(5, 1), stride=1, padding='same')
        self.relu4 = nn.ReLU()
        self.pool4 = nn.MaxPool2d(kernel_size=(2, 1), stride=(2,1))
        self.conv5 = nn.Conv2d(in_channels=10, out_channels=15, kernel_size=(5, 1), stride=1, padding='same')
        self.relu5 = nn.ReLU()
...
class ResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        self.skip_bn = nn.BatchNorm2d(out_channels) if isinstance(self.skip, nn.Conv2d) else nn.Identity()

    def forward(self, x):
        identity = self.skip_bn(self.skip(x))  # Downsample the shortcut if needed
...
class ResNet(nn.Module):
    def __init__(self, n_class=2, n_chan=19, n_samples=64):
        ...
        #self.layer4_block = ResidualBlock(256, 256, stride=2)
    ...
    def forward(self, x):
        ...
        x = self.layer1_block(x)
        x = self.layer2_block(x)
        x = self.layer3_block(x)
        #x = self.layer4_block(x)
